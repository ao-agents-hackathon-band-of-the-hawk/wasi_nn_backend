...
bool load_adapter(const common_params &params) {
    ...
    void populate_token_probs(const server_slot &slot, completion_token_output &result, bool post_sampling, bool special, int idx)
    {
        ...
        else
        {
            ...
            {
                result.probs.push_back({cur[i].id,
    ...
    {
        // Always return true since we've removed multimodal support
        return true;
    }
        ...
        const std::function<bool()> &is_connection_closed)
    {
        ...
        {
            server_task_result_ptr result = queue_results.recv_with_timeout(id_tasks, 1); // 1 second timeout
        ...
        const std::function<bool()> &is_connection_closed)
    {
        ...
        while (true)
        {
            ...
            GGML_ASSERT(
                dynamic_cast<server_task_result_cmpl_partial *>(result.get()) != nullptr || dynamic_cast<server_task_result_cmpl_final *>(result.get()) != nullptr);
